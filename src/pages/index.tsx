...
import { CtyTable } from "@/feature/company-table";
import { TaiSanTable } from "@/feature/tai-san-table";
    ...
    <div className='min-h-dvh w-full overflow-hidden bg-white p-6'>
      {!token ? (
        ...
        <div className='flex flex-col gap-4'>
          ...
          {activeTab === "1" ? <TaiSanTable token={token} /> : <CtyTable token={token} />}
        </div>
      )}
      <Toaster />
    </div>
...
const Header = ({
  ...
  return (
    <Tabs value={activeTab} onValueChange={setActiveTab}>
      <TabsList>
        <TabsTrigger value='1'>Tài Sản</TabsTrigger>
        <TabsTrigger value='2'>Công Ty</TabsTrigger>
      </TabsList>
    </Tabs>
  );
};
